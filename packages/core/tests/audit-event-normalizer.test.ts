...
import {
  ...
  filterAuditEvents,
  buildAuditSummary,
} from "../src/audit/eventNormalizer";
import type {
  AuditEvent,
  AuditEventSourceContext,
  NormalizeAuditEventOptions,
} from "../src/audit/eventNormalizer";
import type {
  RegisteredEvent,
...
function makePaymentExecutedEvent(
  overrides: Partial<PaymentExecutedEvent> = {}
): PaymentExecutedEvent {
  return {
...
function makeSalaryRevealedEvent(
  overrides: Partial<SalaryRevealedEvent> = {}
): SalaryRevealedEvent {
  return {
...
function makePaymentCancelledEvent(
  overrides: Partial<PaymentCancelledEvent> = {}
): PaymentCancelledEvent {
  return {
...
describe("normalizeAuditEvents() — array overload", () => {
  it("normalizes an array of events", () => {
    const events = [makeRegisteredEvent(), makePaymentExecutedEvent(), makeCommittedEvent()];
